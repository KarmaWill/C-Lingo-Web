...
终端会显示局域网地址，例如 `http://192.168.x.x:3000/`。
...
- 确认终端里已显示 `Serving HTTP on ... port 3000`
- 确认访问的是 `http://localhost:3000/`（不是 https）
- 若端口被占用，可换端口：`./start-lan.sh 8080`

**启动时报 `Address already in use` / 端口已被占用**

- 原因：旧的服务进程未退出，占用了 3000 端口且可能已僵死
- 处理：

      kill $(lsof -t -iTCP:3000 -sTCP:LISTEN)
      ./start-lan.sh 3000

- 或换端口：`./start-lan.sh 8080`

**局域网 IP（如 172.26.x.x）在 iPad 上打不开**

- 本机先确认能打开：`http://localhost:3000/`
